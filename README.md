...
Ukkonen is an implementation of Ukkonen's algorithm. Most (all?) of the tutorials and python files on the internet for this seem to fail to consider edge cases. Figuring this out on my own took way too long. It runs in linear time up to inputs of length around a million, the longest my computer can handle. I imagine it's linear for all input lengths, but can't prove it. As far as I can tell, it never throws an error, even with long strings. When using it to find the longest common substring, it gets the right answer by creating a suffix tree for a string of length order 10^5; I think this is decent evidence of correctness. See my rosalind profile https://rosalind.info/users/cammyholt/. The problem that inspired this project was 'lcsm'.
